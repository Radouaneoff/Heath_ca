...
async function AddDoctor(req,res){
    ...
    if(fullname !== '' && phone !== '' && gender !== '' && date !== '' && category !== '' && username !== '' && password !== '' ){
        try {
            ...
            res.render('addDoctor',{error:true, state:true, succesmsg:`The doctors ${fullname} add successfully.`});

        } catch (error) {
            res.render('addDoctor',{error:true,state:false, errormsg:'Something went worng, please try again.'});
            console.log(error);

        }
    }else{
        res.render('addDoctor',{error:true, state:false,errormsg:'Some field are required.'});
    }
